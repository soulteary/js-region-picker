...
window.RegionPicker=function(a,b){function c(g){f.Options&&f.Options.updater&&"function"===typeof f.Options.updater&&(f.Selected=f.Selected.sort(function(e,h){return e.code<h.code?-1:e.code>h.code?1:0}),f.Options.updater(f.Selected,g,f))}function k(){for(var g=f.RegionList.map(function(q){return[(q.name||"").slice(0,1).toUpperCase(),(q.code||"").slice(0,1).toUpperCase()]}).reduce(function(q,r){q[r[0]]=!0;q[r[1]]=!0;return q},{}),e="",h=65;90>=h;h++){var l=String.fromCharCode(h);e=g[l]?e+('<span class="clickable picker-letter" data-code="'+
l+'">'+l+" </span>"):e+('<span class="clickable picker-letter-disabled">'+l+" </span>")}document.querySelector("#"+f.ComponentId+" .region-letters-container").innerHTML=e}function m(){var g=f.RegionList.map(function(e){var h=e.code,l=e.cname;return'\n        <div class="region-picker-item">\n          <label for="region-picker-'+h+'" class="clickable flex flex-row align-center" data-code="'+h+'">\n            <div class="region-checkbox">\n                <input id="region-picker-'+h+'" type="checkbox" name="region-picker" value="'+
h+'" />\n                <span class="region-checkbox-elem"></span>\n            </div>\n            <img src="'+f.BaseDir+"assets/region-icon/"+("uk"===h?"gb":"tw"===h?"cn":h)+'.svg" loading="lazy" alt="'+l+'" />\n            <div class="flex flex-col">\n                <div class="region-picker-item-cname">'+l+'</div>\n                <div class="region-picker-item-name">'+e.name+"</div>\n            </div>\n          </label>\n        </div>"});document.querySelector("#"+f.ComponentId+" .region-list-conatiner").innerHTML=
g.join("")}function n(g){document.querySelectorAll("#"+f.ComponentId+" .region-picker-item").forEach(function(e){if("*"===g)e.classList.remove("hide");else{var h=e.querySelector(".region-picker-item-name").innerText,l=e.querySelector("label").getAttribute("data-code");h.startsWith(g)||l.startsWith(g)?e.classList.remove("hide"):e.classList.add("hide")}})}function p(){var g=f.Selected.map(function(e){var h=e.cname,l=e.code;e.icon="uk"===l?"gb":"tw"===l?"cn":l;return'\n        <div class="region-item-selected flex flex-row flex-center" data-code="'+
l+'">\n          <span>'+h+'</span>\n          <img class="clickable btn-remove-selected" src="'+f.BaseDir+'assets/img/icon-close.svg" alt="" />\n        </div>'});c("change");document.querySelector("#"+f.ComponentId+" .region-selected-container").innerHTML=g.join("")}function t(g){if(g){var e=f.Selected,h=e.findIndex(function(l){return l.code===g});e.splice(h,1);f.Selected=e}}function x(){document.getElementById(f.ComponentId).addEventListener("click",function(g){var e=g.target;e&&(e.closest(".region-selected-container")?
-1!==(e.className||"").trim().indexOf("btn-remove-selected")&&(g=e.parentElement.getAttribute("data-code"),t(g),p(),v()):e.closest(".btn-select-all")?(g.preventDefault(),f.Selected=[],u("all")):e.closest(".btn-clear-all")?(g.preventDefault(),f.Selected=[],f.Options.preselected&&u(f.Options.preselected)):e.closest(".btn-submit")&&(g.preventDefault(),w()))})}function y(){document.querySelector("#"+f.ComponentId+" .region-list-conatiner").addEventListener("click",function(g){(g=g.target.closest("label"))&&
g.matches(".region-picker-item label")&&setTimeout(function(){var e=Array.from(document.getElementsByName("region-picker")).filter(function(h){return h.checked}).map(function(h){return h.value});f.Selected=f.RegionList.filter(function(h){return e.includes(h.code)});p()},10)})}function z(){var g=document.querySelector("#"+f.ComponentId+" .region-letters-container"),e=Array.from(g.querySelectorAll(".picker-letter"));g.addEventListener("click",function(h){h.preventDefault();h=h.target;var l=h.getAttribute("class");
l&&-1!=l.trim().indexOf("picker-letter")&&(l=h.getAttribute("data-code"))&&(h.classList.contains("picker-letter-active")?(h.classList.remove("picker-letter-active"),n("*")):(e.forEach(function(q){return q.classList.remove("picker-letter-active")}),h.classList.add("picker-letter-active"),n(l.toLowerCase())))})}function A(){var g=document.getElementById("region-search-input"),e=document.querySelectorAll("#"+f.ComponentId+" .region-picker-item");g.addEventListener("input",function(){var h=(g.value||
"").trim().toLowerCase();h?(Array.from(document.querySelectorAll("#"+f.ComponentId+" .region-letters-container .picker-letter")).forEach(function(l){return l.classList.remove("picker-letter-active")}),e.forEach(function(l){var q=l.querySelector(".region-picker-item-cname").innerText,r=l.querySelector(".region-picker-item-name").innerText.toLowerCase();q.includes(h)||r.includes(h)?l.classList.remove("hide"):l.classList.add("hide")})):e.forEach(function(l){return l.classList.remove("hide")})})}function w(){var g=
document.getElementById(f.ComponentId);g.className+=" hide";f.Visiable=!1;c("submit")}function u(g){if(g){if("string"==typeof g){var e=g.toLowerCase().trim();f.Selected="all"==e?f.RegionList:f.RegionList.filter(function(l){return e==l.code})}else{var h=g.map(function(l){return l.toLowerCase().trim()});f.Selected=f.RegionList.filter(function(l){return h.includes(l.code)})}p();v()}}function v(){Array.from(document.querySelectorAll("#"+f.ComponentId+" input[name=region-picker]")).forEach(function(g){f.Selected.forEach(function(e){g.value===
e.code&&(g.checked=!0)})})}b=void 0===b?{}:b;var f={ComponentId:"",Options:{},BaseDir:"",Selected:[],RegionList:[],Visiable:!1,GetSelected:function(){return this.Selected},Show:function(){var g=document.getElementById(f.ComponentId);g.className=g.className.replace(/\s?hide/g,"");f.Visiable=!0},Hide:w};return function(g,e){var h="region-picker-"+Math.random().toString(36).slice(-6);f.ComponentId=h;f.Options=e;var l=e.baseDir,q=e.preselected,r=e.visiable;f.RegionList=e.data;f.Visiable=!!r;""!=l&&(f.BaseDir=
l+"/");e='\n        <div id="'+h+'" class="region-picker-container '+(f.Visiable?"":"hide")+'">\n          <div class="flex flex-row align-center justify-between">\n              <div class="region-picker-labels no-select">\u9009\u62e9\u67e5\u8be2\u5730\u533a</div>\n              <div class="flex flex-row align-center">\n                  <div class="clickable no-select btn-select-all">\u5168\u9009</div>\n                  <div class="clickable no-select btn-clear-all">\u91cd\u7f6e</div>\n                  <div class="clickable no-select btn-submit flex flex-center">\u786e\u5b9a</div>\n              </div>\n          </div>\n\n          <div class="region-selected-container no-select flex flex-row flex-wrap"></div>\n\n          <div class="region-search flex flex-row align-center">\n              <input type="text" id="region-search-input" placeholder="\u8fdb\u884c\u533a\u57df\u68c0\u7d22" />\n              <i class="icon-search"></i>\n          </div>\n\n          <div class="region-letters-container no-select "></div>\n          <div class="region-list-conatiner no-select flex flex-row flex-wrap align-start"></div>\n        </div>\n      ';
document.querySelector(g).innerHTML=e;k();m();x();y();z();A();q&&u(q);c("init");return f}(a,b)};}).call(this);
